...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # API Configuration for hybrid system
    ...
    def setup_ui(self):
        ...
        self.accessibility_btn = QPushButton("🔧 🌐")
    ...
    # Around line 855-900 in self-checkout-system/ui/main_window.py
# Replace the broken on_add_to_cart method with this properly indented version:

    # Fix for self-checkout-system/ui/main_window.py
# Find the on_add_to_cart method and replace it with this:

    # Fix for self-checkout-system/ui/main_window.py
# Replace the on_add_to_cart method with this corrected version:

    # Fix for self-checkout-system/ui/main_window.py
# Replace the on_add_to_cart method with this final version:
    ...
    def on_add_to_cart(self):
        ...
        print(f"Sending {len(self.scanner_widget.detected_products)} products to web cart")
        
        # Product ID mapping - matches PyQt IDs to Web Server IDs
        PRODUCT_ID_MAP = {
            # Chips
            "lays_flat_original": "lays-flat-original",
            "lays_nori_seaweed": "lays-nori-seaweed",
            "lays_ridged_original": "lays-ridged-original",
            "snackjack_original": "snackjack-original",
            "tasto_japanese_seaweed": "tasto-japanese-seaweed",
            "tasto_original": "tasto-original",
            # Drinks
            "coca_cola_bottle": "coca-cola-bottle",
            "coca_cola_can": "coca-cola-can",
            "crystal_water": "crystal-water",
            "fanta_fruit_punch": "fanta-fruit-punch",
            "pepsi": "pepsi",
            "sprite": "sprite",
            # Alternative formats that might exist
            "chips_lays_flat_original": "lays-flat-original",
            "chips_lays_nori_seaweed": "lays-nori-seaweed",
            "chips_lays_ridged_original": "lays-ridged-original",
            "chips_snackjack_original": "snackjack-original",
            "chips_tasto_japanese_seaweed": "tasto-japanese-seaweed",
            "chips_tasto_original": "tasto-original",
            "drinks_coca_cola_bottle": "coca-cola-bottle",
            "drinks_coca_cola_can": "coca-cola-can",
            "drinks_crystal_water": "crystal-water",
            "drinks_fanta_fruit_punch": "fanta-fruit-punch",
            "drinks_pepsi": "pepsi",
            "drinks_sprite": "sprite"
        }
        ...
        for product in self.scanner_widget.detected_products:
            try:
                # Get the product ID and clean it
                product_id = str(product.id).lower().strip()
                print(f"Original product ID from scanner: '{product_id}'")
                print(f"Product name: {product.name}")
                
                # Check if we have a direct mapping
                if product_id in PRODUCT_ID_MAP:
                    web_product_id = PRODUCT_ID_MAP[product_id]
                    print(f"Found in mapping: {product_id} -> {web_product_id}")
                else:
                    # Try to auto-format: remove category prefix if present, convert underscores to dashes
                    # Remove common prefixes
                    for prefix in ["chips_", "drinks_", "chips-", "drinks-"]:
                        if product_id.startswith(prefix):
                            product_id = product_id[len(prefix):]
                            break
                    
                    # Convert underscores to dashes
                    web_product_id = product_id.replace("_", "-")
                    print(f"Auto-formatted: {product_id} -> {web_product_id}")
                
                # Send the request
                request_data = {
                    "product_id": web_product_id,
                    "quantity": 1
                }
                
                print(f"Sending to web API: {request_data}")
                ...
                if response.status_code == 200:
                    ...
                    print(f"✓ Successfully added {product.name} to web cart")
                ...
                errors.append("Cannot connect to web server")
                print("Connection error - is the web server running?")
                break
            except Exception as e:
                errors.append(f"{product.name}: {str(e)}")
                print(f"Unexpected error sending {product.name}: {e}")
                import traceback
                traceback.print_exc()
        ...
        if success_count > 0:
            ...
            msg.setText(f"✅ Sent {success_count} item(s) to web cart!\n\n"
                       f"Open browser to view cart at:\nhttp://localhost:8000/cart")
            
            if errors:
                error_text = "\n".join(errors[:3])
                if len(errors) > 3:
                    error_text += f"\n... and {len(errors)-3} more"
                msg.setText(msg.text() + f"\n\n⚠️ Some items failed:\n{error_text}")
            
            # Clear detected products after success
            self.scanner_widget.clear_detected()
            
        
        msg.exec_()

# Also add this method if you don't have update_add_button:
    def update_add_button(self):
        """Update add button state based on detected products"""
        has_products = len(self.scanner_widget.detected_products) > 0
        self.add_btn.setEnabled(has_products)
        
        if has_products:
            count = len(self.scanner_widget.detected_products)
            self.add_btn.setText(f"🌐 Send {count} item(s) to Web")
        else:
            self.add_btn.setText("🌐 Send to Web Cart")
    ...
    def on_checkout(self):
        ...
        msg.setText(f"Total: ฿{summary['total']:.2f}\n\nProceed with payment?")
