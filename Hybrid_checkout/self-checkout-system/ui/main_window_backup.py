...
class MainWindow(QMainWindow):
    ...
    def on_add_to_cart(self):
        ...
        for product in self.scanner_widget.detected_products:
            try:
                response = requests.post(
                    ...
                    json={"product_id": product.id.replace("_", "-") if "_" in product.id else f"{product.category}-{product.id}"},
                    timeout=self.api_timeout
                )
                
                if response.status_code == 200:
                    print(f"✓ Added {product.name} to web cart")
                    success_count += 1
                else:
        ...
        if success_count > 0:
            ...
            msg.setText("Failed to send items:\n" + "\n".join(errors[:3]))
            msg.exec_()
